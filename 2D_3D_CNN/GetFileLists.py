#List_IDs
#Two conditions : same percentage of patients with disease, with and without multiplication by number of slices
...
def GetFileLists(params, shuffle=True):
    ...
    while check_slice is False & (t<20):
        ...
        partition["train"] = slicing(partition["train"], 
            df, shuffle, params)
        partition["validation"] = slicing(partition["validation"], df, shuffle, params)
        if (abs(len(partition["train"])/(len(partition["train"])+len(partition["validation"])) - len_train/len_tot)> 0.05) :
            check_slice = False
            continue
        check_slice = check_class_balance(partition, params, balance = (0.15,0.25))
        ...
        if len(params["distance"]) == 2:
            for n in range(n_slices):
                if np.sum(seg_array[n,:,:]) > 0.2 * max_area:
                    print(ID, n, np.sum(seg_array[n,:,:]), max_area)
                    x.append([ID,n])
                    temp.append(ID)
