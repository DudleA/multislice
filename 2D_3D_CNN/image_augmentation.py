...
Main modification: change order of angles in 3D rotation
...
def deform_pixel(X, Y=None, alpha=15, sigma=3):
    ...
    """
    shape = X.shape
    dx = gaussian_filter(np.random.randn(*shape[:-1]), sigma, mode="constant",
                         cval=0) * alpha  # originally with random_state.rand * 2 - 1
    dy = gaussian_filter(np.random.randn(*shape[:-1]), sigma, mode="constant", cval=0) * alpha
    if len(shape) == 3:
        x, y = np.meshgrid(np.arange(shape[0]), np.arange(shape[1]), indexing='ij')
        indices = x + dx, y + dy

    elif len(shape) == 4:
        dz = gaussian_filter(np.random.randn(*shape[:-1]), sigma, mode="constant", cval=0) * alpha
    ...
    if Y is None:
        for i in range(shape[-1]):
            X[..., i] = map_coordinates(X[..., i], indices, order=3).reshape(shape[:-1])
        return X, None
    else:
        for i in range(X.shape[-1]):
            X[..., i] = map_coordinates(X[..., i], indices, order=3).reshape(shape[:-1])
        for i in range(Y.shape[-1]):
            Y[..., i] = map_coordinates(Y[..., i], indices, order=0).reshape(shape[:-1])
        return X, Y
...
def deform_grid(X, Y=None, sigma=25, points=3):
    ...
    shape = X.shape
    if len(shape) == 3:
        coordinates = np.meshgrid(np.arange(shape[0]), np.arange(shape[1]),
    ...
    for i in range(len(shape[:-1])):  # creates the deformation along each dimension and then add it to the coordinates
        if i != 2:  # not deforming in the z direction due to the asymetry in the size
            yi = np.random.randn(*grid) * sigma  # creating the displacement at the control points
            y = map_coordinates(yi, xi, order=3).reshape(shape[:-1])
    ...
    if Y is None:
        for i in range(shape[-1]):
            X[..., i] = map_coordinates(X[..., i], coordinates, order=3).reshape(shape[:-1])
        return X, None
    else:
        for i in range(X.shape[-1]):
            X[..., i] = map_coordinates(X[..., i], coordinates, order=3).reshape(shape[:-1])
        for i in range(Y.shape[-1]):
            Y[..., i] = map_coordinates(Y[..., i], coordinates, order=0).reshape(shape[:-1])
        return X, Y


# The following is adapted by fdubost and fcalvet from https://github.com/keras-team/keras/blob/master/keras/preprocessing/image.py under MIT license
...
def random_transform(x, y=None,
    ...
    if len(x.shape) == 3:
        ...
        rotation_matrix = np.array([[cb * ca, -cb * sa, sb, 0],
                                    [cg * sa + sg * sb * ca, cg * ca - sg * sb * sa, -sg * cb, 0],
                                    [sg * sa - cg * sb * ca, sg * ca + cg * sb * sa, cg * cb, 0],
                                    [0, 0, 0, 1]])
    ...
    if y is None:
        ...
        if z_flip and len(x.shape) == 4:
            print("z flip")
            if np.random.random() < 0.5:
                x = flip_axis(x, img_z_index)
        elif z_flip:
            raise ValueError("can't zflip a 2D image")
        return x, None
