"""
2D CNN based on the ResNet architecture
"""
...
def mini_resnet(input_shape, activation=relu, scalars=False, num_classes=2):
    nfeat = 16
    dil_rate = 1
    activ = activation
    drop_rate = 0.3
    ...
    conv1 = Conv2D(nfeat, (3, 3), kernel_initializer=init,
    ...
    conv2 = Conv2D(nfeat * 2, (3, 3), kernel_initializer=init,
        strides = 2, padding='same', name="conv2")(drop1)
    bn2 = BatchNormalization(name="bn2")(conv2)
    activ2 = Activation(activ, name="activ2")(bn2)
        
    conv3 = Conv2D(nfeat * 2, (3, 3), kernel_initializer=init,
        padding='same', dilation_rate=dil_rate, name="conv3")(activ2)
    bn3 = BatchNormalization(name="bn3")(conv3)
    
    conv_b1 = Conv2D(nfeat * 2, (1, 1), kernel_initializer=init, 
        strides = 2, padding='same', name="conv_b1")(drop1)
    ...
    conv4 = Conv2D(nfeat * 4, (3, 3), kernel_initializer=init, 
        strides = 2, padding='same', name="conv4")(drop2)
    bn4 = BatchNormalization(name="bn4")(conv4)
    activ4 = Activation(activ, name="activ4")(bn4)
        
    conv5 = Conv2D(nfeat * 4, (3, 3), kernel_initializer=init,
        padding='same', dilation_rate=dil_rate, name="conv5")(activ4)
    bn5 = BatchNormalization(name="bn5")(conv5)
        
    conv_b2 = Conv2D(nfeat * 4, (1, 1), kernel_initializer=init, 
        strides = 2, padding='same', name="conv_b2")(drop2)
    
    skip1 = Add(name="skip1")([bn5, conv_b2])
    activ5 = Activation(activ, name="activ5")(skip1)

    gap = GlobalAveragePooling2D(name = "gap")(activ5)
