...
if model_load is False:
    prefix = str(datetime.date.today())+'-'+str(datetime.datetime.now())[11:16]
    ...
    roc_callback = roc_val(callback_generator, params["savefolder"], 
        prefix + 'roc_callback.csv', label=params["label"])
    
    'Exponential moving average callback'
    ema_callback = ExponentialMovingAverage(input_shape, test_generator, 
        params["savefolder"], prefix+'ema_callback.csv',
        scalars = params["scalars"], num_classes = num_classes)
    ...
    ema_valfiles = glob.glob(os.path.join(params["savefolder"], 
        "*ema_callback.csv"))
    ema_valfile0 = ema_valfiles[0]
    ema_callback = ExponentialMovingAverage(input_shape, 
        test_generator, params["savefolder"], ema_valfile0[-32:], 
        append = True, start_epoch = start_epoch, 
        scalars = params["scalars"], num_classes = num_classes)
...
model.fit_generator(generator=training_generator,
                    ...
                    callbacks=[roc_callback, ema_callback,
                    csv_logger, checkpoint, checkpoint_best])
