...
def correct_center(center, seg_array, distance, center_seg):
    ...
    small_dx, small_dy = int(0.9 * dx), int(0.9 * dy)
    while (abs(sum_seg - np.sum(seg_array[x - small_dx:x + small_dx, 
        y - small_dy:y + small_dy, :])) > 0.01 * sum_seg):
        t += 1

        if ((np.ceil(abs(y_seg - y) / 10) == 0) & 
            (np.ceil(abs(x_seg - x) / 10) == 0)) | (t > 20):
            raise ValueError("Seg doesn't fit in crop")

        elif y - dy < np.ceil((y - y_seg) / 10):
            x += np.sign(x_seg - x) * np.ceil(abs(x_seg - x) / 10).astype(int)
        elif y_max - dy - y < np.ceil((y_seg - y) / 10):
            x += np.sign(x_seg - x) * np.ceil(abs(x_seg - x) / 10).astype(int)
        elif x - dx < np.ceil((x - x_seg) / 10):
            y += np.sign(y_seg - y) * np.ceil(abs(y_seg - y) / 10).astype(int)
        elif x_max - dx - x < np.ceil((x_seg - x) / 10):
            y += np.sign(y_seg - y) * np.ceil(abs(y_seg - y) / 10).astype(int)
        else:
            x += np.sign(x_seg - x) * np.ceil(abs(x_seg - x) / 10).astype(int)
            y += np.sign(y_seg - y) * np.ceil(abs(y_seg - y) / 10).astype(int)
        ...
        if (x - dx < 0) | (x + dx > x_max) | (y - dx < 0) | (y + dy > y_max):
            raise ValueError("Crop outside of image, center: ", x, y, 
                ", max: ", x_max, y_max)
