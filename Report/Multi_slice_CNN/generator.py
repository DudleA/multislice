...
Adapted in May 2019 by A. Dudle for the multi-slice CNN
...
class DataGenerator(keras.utils.Sequence):
    ...
    def prepare_batch(self, list_IDs):
        ...
        if self.params["scalars"] is True:
            ...
            scalars = scalarsReadFunction(ID, self.params["source"], 
                data=self.params["data"], shape = x.shape)
            S.append(scalars)
    ...
    def save_images(self, X, Y, list_IDs, predict=False, overlay=False, 
        epoch_number=None):
        """
        Save a png to disk (params["savefolder"]) to illustrate the data 
            being generated
        predict: if set to True allows the saving of predicted images, 
            remember to set Y and list_IDs as None
        the to_predict function can be used to reset the counter of 
            saved images, this allows if shuffle is False to have the 
            same order between saved generated samples and the 
            predicted ones
        """
        if predict:
            genOrPred = "predict_"
        ...
        if self.plotgenerator > self.plotedgenerator and (
                X[0].shape[-1] == 2 or X[0].shape[-1] >= 4 or (
                Y is not None and (Y[0] is not None and 
                Y[0].shape[-1] == 2))):
        ...
        if self.plotgenerator > self.plotedgenerator:
            ...
            if len(X[0].shape) == 4:
                '''
                Save augmented images for 3D (will save 10 slices from 
                a single volume)
                '''
                Xto_print = X[0]
